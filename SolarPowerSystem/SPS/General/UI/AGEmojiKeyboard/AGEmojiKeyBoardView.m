...
static const CGFloat ButtonHeight = 45;//37;
...
@property (nonatomic) AGEmojiSegmentedControl *mySegmentsBar;
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource {
    ...
    if (self) {
        ...
        self.mySegmentsBar = [[[AGEmojiSegmentedControl nib] instantiateWithOwner:nil options:0] firstObject];
        self.mySegmentsBar.frame = CGRectMake(0, self.height - 37, CGRectGetWidth(self.bounds), 37);
        self.mySegmentsBar.selectedSegmentIndex = self.defaultSelectedCategory;
        
        @weakify(self)
        self.mySegmentsBar.blockSendPressed = ^(id data){
            @strongify(self)
            if (self.blockSendPressed) {
                self.blockSendPressed(nil);
            }
        };
        
        self.mySegmentsBar.blockBtnSelectChanged = ^(id selectedSegmentIndex){
            @strongify(self)
            self.category = [self categoryNameAtIndex:[selectedSegmentIndex intValue]];
            self.pageControl.currentPage = 0;
            [self setNeedsLayout];
        };
        [self addSubview:self.mySegmentsBar];
        
        
        self.pageControl = [UIPageControl autoLayoutView];//[[UIPageControl alloc] init];
        self.pageControl.hidesForSinglePage = YES;
        self.pageControl.currentPage = 0;

        self.pageControl.pageIndicatorTintColor = [UIColor colorWithHexString:@"#cccccc"];
        self.pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"#999999"];
        
        CGSize pageControlSize = [self.pageControl sizeForNumberOfPages:3];
        CGSize frameSize = CGSizeMake(CGRectGetWidth(self.bounds),
                                      CGRectGetHeight(self.bounds) - CGRectGetHeight(self.mySegmentsBar.bounds) - pageControlSize.height);
        ...
        [self.pageControl pinToSuperviewEdges:(JRTViewPinLeftEdge) inset:0];
        [self.pageControl pinToSuperviewEdges:(JRTViewPinRightEdge) inset:0];
        [self.pageControl pinAttribute:NSLayoutAttributeBottom toAttribute:NSLayoutAttributeTop ofItem:self.mySegmentsBar];
        
        CGRect scrollViewFrame = CGRectMake(0,
                                            10,//CGRectGetHeight(self.segmentsBar.bounds),
        ...
        [line pinToSuperviewEdges:(JRTViewPinTopEdge) inset:0];
        [line pinToSuperviewEdges:(JRTViewPinLeftEdge) inset:0];
        [line pinToSuperviewEdges:(JRTViewPinRightEdge) inset:0];
        [line constrainToHeight:.5];
...
- (void)layoutSubviews {
    ...
    CGSize pageControlSize = [self.pageControl sizeForNumberOfPages:3];
    NSUInteger numberOfPages = [self numberOfPagesForCategory:self.category
                                                  inFrameSize:CGSizeMake(CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - CGRectGetHeight(self.mySegmentsBar.bounds) - pageControlSize.height)];
